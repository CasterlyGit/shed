...
# Handoff ID = "hf-" + first 8 chars of session UUID (stable, unique per session)
HF_ID="hf_${SID:0:8}"

python3 /Users/casterly/.claude/scripts/generate-handoff.py "$TRANSCRIPT" "$SID" "$HF_ID" 2>/dev/null || true
...
find "${HOME}/.claude/state" -maxdepth 1 -name "handoff-hf_*.md" -mtime +3 -delete 2>/dev/null || true
